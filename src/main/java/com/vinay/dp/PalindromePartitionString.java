...
public class PalindromePartitionString {

    public static void main(String[] args) {

        /*String str = "abaa";
        int i = minPartition(str);
        System.out.println(i);*/
        //str = "ababbbabbababa";
          int i = minPartition("ababbbabbababa");
        System.out.println(i);
    }

    private static int minPartition(String str) {
        int n = str.length();
        ...
        for (int i = 2; i <= n; i++) {
            for (int a = 0; a < (n - i + 1); a++) {
                ...
                if (i == 2)
                    pal[a][b] = str.charAt(a) == str.charAt(b);
                else {
                    pal[a][b] = str.charAt(a) == str.charAt(b) && pal[a + 1][b - 1];
                }
                ...
                if (pal[a][b])
                    ...
                    count[a][b] = Integer.MAX_VALUE;
                    for (int k = a; k <= b - 1; k++) {
                        count[a][b] = Math.min(count[a][b], count[a][k] + count[k + 1][b] + 1);
                    }
